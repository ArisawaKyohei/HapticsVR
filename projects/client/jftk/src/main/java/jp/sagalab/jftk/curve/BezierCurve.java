...
public class BezierCurve extends ParametricCurve {
  ...
  public BezierCurve differentiate() {
  	...
  	Point pre = controlPoints[0];
  	Point[] differentiate = new Point[degree];
  	for (int i = 1; i < controlPoints.length; ++i) {
	  Point current = controlPoints[i];
	  differentiate[i - 1] = Point.createXYZ(
		degree * (current.x() - pre.x()),
		degree * (current.y() - pre.y()),
		degree * (current.z() - pre.z())
	  );
	  pre = current;
  	}
  	return BezierCurve.create(differentiate, range());
  }
