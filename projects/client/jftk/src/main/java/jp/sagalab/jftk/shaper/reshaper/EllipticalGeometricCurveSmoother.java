...
public class EllipticalGeometricCurveSmoother implements FuzzyObjectReshaper {
   ...
   */
  public static EllipticalGeometricCurveSmoother create(PrimitiveCurve[] _primitives, Point[] _separatePoints, GridSpace _grid, boolean _isClosed) {
    if (_primitives == null) {
    ...
    return new EllipticalGeometricCurveSmoother(_primitives, _separatePoints, _grid, _isClosed);
  }
  ...
  public ShapedResult reshape() {
    ...
    if (m_isClosed) {
      BezierCurve[] beziers = ((OpenFreeCurve)primiti).beziers();
      Point[] lastCP = beziers[beziers.length - 1].controlPoints();
      lastCP[lastCP.length - 1] = beziers[0].controlPoints()[0];
      beziers[beziers.length - 1] = BezierCurve.create(lastCP, beziers[beziers.length - 1].range());
      primiti = ClosedFreeCurve.create(beziers);
    }
  ...
  private EllipticalGeometricCurveSmoother(PrimitiveCurve[] _primitives, Point[] _separatePoints, GridSpace _grid, boolean _isClosed) {
