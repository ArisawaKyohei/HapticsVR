...
import jp.sagalab.jftk.recognition.*;
...
public class ExperimentObjectSnapper implements FuzzyObjectSnapper {
     ...
     */
    public static ExperimentObjectSnapper create(FuzzyGridSnapper<Point> _gridSnapper) {
        if (_gridSnapper == null) {
            throw new IllegalArgumentException(" GridSnapper is null. ");
        }
        return new ExperimentObjectSnapper(_gridSnapper);
    }
    ...
    public ShapedResult snap(RecognitionResult _recognition, GridSpace _grid, SplineCurve _fsc) {
        ...
        final BoundingBox box = null;
        final PrimitiveType type = _recognition.getType();

        // 閉曲線、自由曲線の場合、端点スナッピングは行わない
        if (type == PrimitiveType.ELLIPTIC_ARC
        ...
        Point[] p = ((QuadraticBezierCurve)model.getCurve()).controlPoints();
        double w = ((QuadraticBezierCurve)model.getCurve()).weight();
        Point o = p[0].internalDivision(p[2], 1, 1).internalDivision(p[1], -w, 1);
        // スナッピング前の特徴点
        Point[] snappingPoints = (type == PrimitiveType.CIRCULAR_ARC || type == PrimitiveType.LINE) ?
                new Point[] {model.getStart(), model.getEnd()} : new Point[]{model.getStart(), o};
        // 法線ベクトルを取得
    ...
    public Map<Fragment, ShapedResult> snap(Map<Fragment, RecognitionResult> _recognitions, List<Fragment> _fragments, GridSpace _gridSpace) {
        ...
        Map<Fragment, ShapedResult> result = new HashMap<>();
     ...
     */
    private static Vector calcSnappedNormal(QuadraticBezierCurve _curve, FuzzyGridSnapper<Point> _snapper, GridSpace _grid) {
    ...
    private ExperimentObjectSnapper(FuzzyGridSnapper<Point> _gridSnapper) {
