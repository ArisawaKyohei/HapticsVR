...
public class MFGSSnapper implements FuzzyGridSnapper<Point> {
  ...
  public GridPoint snap(GridSpace _grid, Point _p) {
    ...
    // 解像度を最低解像度に設定
    GridSpace snappedGrid = grids[0];
    // スナッピング点の設定
    Point snapped = nearestPoint( snappedGrid, _p );
    // ファジィグリッド格子点がスナップ対象の点に含まれている必然性値
    double nec = snapped.includedIn( _p ).necessity();
    // 否定の最小値
    double minInvGrade = 1;
    // グレードの導出
//		double grade = Math.min( m_true.calculate( nec ), minInvGrade );
    double grade = Math.min( nec , minInvGrade );
    // グレードの最大値
    double maxGrade = grade;
    // 否定の最も小さなものを保持
//		minInvGrade = Math.min( minInvGrade, m_false.calculate( nec ) );
    minInvGrade = Math.min( minInvGrade, 1 - nec  );

    // 最低解像度以外のグレード導出
    for ( int i = 1; i < grids.length; ++i ) {
      // スナッピング対象の点に最も近いファジィグリッド格子点を生成
      Point gridPoint = nearestPoint( grids[i], _p );
      // ファジィグリッド格子点がスナップ対象の点に含まれている必然性値
      nec = gridPoint.includedIn( _p ).necessity();
      // グレード値の導出
//			grade = Math.min( m_true.calculate( nec ), minInvGrade );
      grade = Math.min( nec , minInvGrade );
...
//			minInvGrade = Math.min( minInvGrade, m_false.calculate( nec ) );
    ...
    if ( maxGrade < minInvGrade ) {
      snapped = _p;
      snappedGrid = null;
    }

    return GridPoint.create( snapped, snappedGrid );
  }
